...
`dispatcherList`属性在`BrokerController`类的`initialize()`方法和`DefaultMessageStore`类的构造函数被初始化，代码如下：
```java
...
`CommitLogDispatcherCalcBitMap`类用于根据消息计算布隆过滤器的值，关于该类的实现，可以看笔记[如何实现消息过滤](../client/如何实现消息过滤.md)。
...
对于事务消息相关的处理可以看笔记[如何实现事务消息](../client/如何实现事务消息.md)，对于普通消息，由`DefaultMessageStore`类的`putMessagePositionInfo()`方法处理，代码如下：
```java
...
以上是`CommitLogDispatcherBuildConsumeQueue`类为commitlog文件中的消息添加consumequeue记录的过程，下面在分析`CommitLogDispatcherBuildIndex`类构建消息索引的过程，`CommitLogDispatcherBuildIndex`源码如下：
```java
...
只是简单的调用`IndexService`类的`buildIndex()`方法，该方法代码：
```java
...
`buildIndex()`方法获取或创建一个`IndexFile`对象，该对象表示一个index文件，创建过程可以看`retryGetAndCreateIndexFile()`方法的源码和源码中写的注释，这里不再赘述。获取到`IndexFile`对象后，`buildIndex()`方法调用`putKey()`方法构建消息的索引，构建时的key为根据消息的UniqKey和消息的KEYS属性计算得到的字符串，对应的构建方法是`buildKey()`方法，代码：
```java
...
`putKey()`方法的实现原理已经在注释中说明了，简单来说，index文件保存消息索引的方式和JDK7的HashMap类似，都是数组加链表，只不过index文件是通过数组表示的链表。

以上是构建索引的过程，对于根据索引查找消息，对应的方法是`selectPhyOffset()`，该方法能够根据key和时间范围查找索引中的消息，代码：
```java
...
`selectPhyOffset()`方法的实现原理也很简单，根据key的hashCode定位到hash槽，再遍历链表判断hashCode是否相等即可。
