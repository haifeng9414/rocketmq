`NettyRemotingServer`的父类`NettyRemotingAbstract`及实现的接口`RemotingServer`分别在[NettyRemotingAbstract的实现](NettyRemotingAbstract类的实现.md)和[RemotingServer接口](RemotingServer接口.md)做了分析，下面`NettyRemotingServer`类的分析假定已经对这两个笔记有了全面的了解，这里只对`NettyRemotingServer`类新增的实现做分析。

需要`NettyRemotingServer`类实现的方法如下，这些方法的作用在[NettyRemotingAbstract的实现](NettyRemotingAbstract类的实现.md)和[RemotingServer接口](RemotingServer接口.md)都有介绍：
```java
...
这里先放下`NettyRemotingServer`类的源码，源码中已经有了比较详细的注释，`NettyRemotingServer`类的实现在源码后分析：
```java
...
### `start()`和`shutdown()`方法
`NettyRemotingServer`类使用netty完成端口监听，所以`start()`方法主要工作是完成netty的`ServerBootstrap`对象的创建和启动：
```java
@Override
public void start() {
    // 处理childChannel的EventExecutorGroup，serverWorkerThreads默认为8
...
### `registerProcessor()`、`registerDefaultProcessor()`和`getProcessorPair()`方法
这两个方法的作用在[NettyRemotingAbstract的实现](NettyRemotingAbstract类的实现.md)中有说明，用于注册处理请求的处理器：
```java
@Override
public void registerProcessor(int requestCode, NettyRequestProcessor processor, ExecutorService executor) {
    ExecutorService executorThis = executor;
...
// 对于RocketMQ的事务消息，服务器需要向客户端询问事务状态，所以服务器也需要实现发送请求的相关方法，而发送请求的过程在父类已经实现了，这里只需要简单的调用即可
@Override
public RemotingCommand invokeSync(final Channel channel, final RemotingCommand request, final long timeoutMillis)
        throws InterruptedException, RemotingSendRequestException, RemotingTimeoutException {
    return this.invokeSyncImpl(channel, request, timeoutMillis);
...
@Override
public ChannelEventListener getChannelEventListener() {
    // 默认实现为NettyConnectManageHandler类，用于监听channel的连接、关闭、异常和空闲事件
    return channelEventListener;
}
...
@Override
public ExecutorService getCallbackExecutor() {
    // 通过JDK的Executors创建的线程池，当注册NettyRequestProcessor时没有指定ExecutorService的话用publicExecutor作为ExecutorService
    return this.publicExecutor;
}
```